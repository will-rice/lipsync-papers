...
13      $`\theta\leftarrow{\Theta_{e\hspace{0pt}g} \cup \theta_{c}}`$ // assemble *G* with trainable $`\theta_{c}`$ ;
...
Figure 6: Controller architecture. We show the details of the architecture of *C* in (a) and the sub-module $`_{T}`$ in (b). The controller takes as input 3DMM coefficients and output motion code in $`\mathcal{W}^{+}`$ space\[39\].
