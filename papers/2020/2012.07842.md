...
where, $`\lambda`$ is the weight for perceptual loss and $`F^{(i)}`$ is the ith layer of VGG19 network with M_(i) elements of VGG layer.
...
where, v_(n) and a_(n) are fc₇ vectors for video and audio inputs respectively. y $`\epsilon`$ \[0,1\] is the binary similarity metric for video and audio input.
...
where, p_(i) is described in Figure  3. We have taken the L1 loss of eye aspect ratio(EAR) between real image m_(r) and synthesized frame m_(g).
